                   ...
                   const float* const filter, const int filterWidth)
{
	...
	for(int filter_r = -halfWidth; filter_r <= halfWidth; ++filter_r)
	{
		for(int filter_c = -halfWidth; filter_c <= halfWidth; ++filter_c)
		{
			...
			float3 value;
			value.x = static_cast<float>(inputChannel[idx].x);
			value.y = static_cast<float>(inputChannel[idx].y);
			value.z = static_cast<float>(inputChannel[idx].z);
	...
	outputChannel[thread_1D_pos].x = static_cast<unsigned char>(result.x);
	outputChannel[thread_1D_pos].y = static_cast<unsigned char>(result.y);
	outputChannel[thread_1D_pos].z = static_cast<unsigned char>(result.z);
  // TODO
